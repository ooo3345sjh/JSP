...
public class User2DAO extends DBHelper {
	...
	public void insertUser2(User2VO vo) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("INSERT INTO `user2` VALUES(?,?,?,?)");
	...
	public User2VO selectUser2(String uid) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("SELECT * FROM `user2` WHERE `uid`=? ");
	...
	public void updateUser2(User2VO vo) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("UPDATE `user2` SET `name`=?, `hp`=?, `age`=? WHERE `uid`=?");
	...
	public void deleteUser2(String uid) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("DELETE FROM `user2` WHERE `uid`=?");
			psmt.setString(1, uid);

...
public class User1DAO extends DBHelper {
	...
	public void insertUser1(User1VO vo) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("INSERT INTO `user1` VALUES (?,?,?,?)");
	...
	public User1VO selectUser1(String uid) {
		...
		try {
			con = getConnection();
			psmt = con.prepareStatement("SELECT * FROM `user1` WHERE `uid`=?");
	...
	public void updateUser1(User1VO vo) {
		try {
			con = getConnection();
			psmt = con.prepareStatement("UPDATE `user1` SET `name`=?, `hp`=?, `age`=? WHERE `uid`=?");
	...
	public void deleteUser1(String uid) {
		try {
			con = getConnection();
			psmt = con.prepareStatement("DELETE FROM `user1` WHERE `uid`=?");
			psmt.setString(1, uid);
			psmt.executeUpdate();

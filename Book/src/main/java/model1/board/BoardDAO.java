 ...
 * 이름 : 서정현
 * 내용 : BoardDAO
 * 	- 게시물 목록 CRUD용 DAO(P267)
 * 	- DAO에 selectCount() 추가(P269) 
 * 	- DAO에 selectList() 추가(P271) 
 */
public class BoardDAO extends JDBConnect{
	...
	// 검색 조건에 맞는 게시물 목록을 반환합니다.(페이징 기능 지원)(P319)
	...
	// DAO에 게시물 조회 메서드 추가(P288)
	// 지정한 게시물을 찾아 내용을 반환합니다.
	public BoardDTO selectview(String num) {
		BoardDTO dto = new BoardDTO();
		
		// 쿼리문 준비
		String sql = "SELECT B.*, M.name FROM `member` M"
					 + " JOIN `Board` B "
					 + " ON M.id = B.id "
					 + " WHERE B.num =? ";
